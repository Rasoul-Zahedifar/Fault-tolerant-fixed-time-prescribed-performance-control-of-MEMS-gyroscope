function output=MEMS_finiteControl(in)

global k1 k3 landa beta delta kesi pp p0 pinf taw eps ll
...
alpha=q1desdot-taw*e1.*(1-(pinf./p))+eps*ll.*p.*([1 1]-H.^2).*(-(0.5^0.75)*k1*epsilon.^0.5);
alphadot=q1desddot-taw*e1dot.*(1-(pinf./p))-taw*e1.*(pinf.*pdot./p.^2)+eps*ll.*pdot.*([1 1]-H.^2).*(-(0.5^0.75)*k1*epsilon.^0.5)+eps*ll.*p.*(2*H.*Hdot./([1 1]-H.^2).^2).*(-(0.5^0.75)*k1*epsilon.^0.5)+eps*ll.*p.*([1 1]-H.^2).*(-(0.5^0.75)*0.5*k1*epsilondot.*epsilon.^(-0.5));

e2=q2-alpha;


uprim=alphadot-gamahat-sigmahat-dhat-epsilon.*(1/eps).*(1./p).*(1./ll).*(1./([1 1]-H.^2))-0.5^(0.75)*k3*e2.^0.5;
u=pp*(uprim./rohat);
...
gamahatdot=e2-landa*gamahat;
sigmahatdot=e2-beta*sigmahat;
dhatdot=e2-delta*dhat;
rohatdot=e2.*uprim./rohat-kesi.*rohat;
